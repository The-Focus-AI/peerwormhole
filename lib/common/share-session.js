...
export class ShareSender extends SimpleEmitter {
  ...
  async streamTo(peerId) {
    ...
    const meta = await sendBufferChunks(connection, {
      buffer: this.buffer,
      fileName: this.fileName,
      mimeType: this.mimeType,
      user: this.user,
    ...
    // Wait for the receiver to acknowledge receipt before closing
    await this.waitForAck(meta.id, 30000);
    this.emit('complete', { peerId });
  }

  waitForAck(transferId, timeout = 30000) {
    return new Promise((resolve) => {
      const off = this.mesh.on('message', ({ message }) => {
        if (message?.type === MESSAGE_TYPES.fileAck && message.id === transferId) {
          off();
          clearTimeout(timer);
          resolve();
        }
      ...
      // Fall back to timeout so we don't hang forever if receiver is old version
  ...
  constructor({
    ...
    this.assembler = createTransferAssembler({
      ...
      onComplete: ({ meta, bytes, context }) => {
        this.completed = true;
        // Send ACK back to sender so it knows we received everything
        this.mesh.sendTo(this.senderPeerId, {
          type: MESSAGE_TYPES.fileAck,
          id: meta.id
        });
